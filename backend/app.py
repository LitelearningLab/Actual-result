...
from others.exams import (
    add_exam,
    autosave_exam_answers,
    get_active_exam_status,
    get_exam_details,
    get_exam_list,
    launch_exam_details,
    submit_exam_answers,
...
from others.examschedule import (
    add_exam_schedule,
    get_exam_schedule_details,
...
from others.questions import (
    add_question,
    get_questions_details,
    bulk_upload_questions,
    create_question_using_llm,
    fine_tune_questions_using_llm,
)
...
from masters.location import (
    get_location_hierarchy_details,
    get_registered_countries_details,
)
...
from others.demo_request import (
    submit_demo_request,
    get_demo_requests,
    get_demo_request_by_id,
...
from others.settings import (
    get_ai_confidence_threshold_response,
...
def normalize_global_scope_request():
    ...
    if path in GLOBAL_SCOPE_EXCLUDED_EXACT_PATHS or any(
        path.startswith(p) for p in GLOBAL_SCOPE_EXCLUDED_PATHS
    ):
        return None

    scoped_institute_id = request.headers.get("X-Institute-Id") or request.headers.get(
        "X-Global-Institute-Id"
    )
    ...
    for key in ("institute_id", "institute"):
        existing_values = [
            str(v) for v in query_values.get(key, []) if str(v or "").strip()
        ]
...
def get_pagination():
    return (
        request.args.get("pageNumber", 1, type=int),
...
def ai_confidence_threshold_route():
    ...
    if request.method == "GET":
        ...
        response_data, status_code = update_ai_confidence_threshold(
...
def delete_page(page, id):
    ...
    if page == "institute":
        ...
        return (
            jsonify({"statusMessage": f"Unknown page '{page}'", "status": False}),
            400,
        )
    return jsonify(response_data), status_code
...
def manage_page(page, action, uuid):
    ...
    if action not in ["activate", "deactivate"]:
        return (
            jsonify(
                {
                    "error": f"Invalid action '{action}'. Use 'activate' or 'deactivate' ."
                }
            ),
            400,
        )
...
def autosave_exam_route():
    ...
    if not current_user:
        return (
            jsonify({"statusMessage": "Authenticated user not found", "status": False}),
            401,
        )
    response_data, status_code = autosave_exam_answers(
        request.json or {}, current_user.user_id
    )
    return jsonify(response_data), status_code
...
def user_dashboard_route():
    ...
    if current_user and not is_super_admin(current_user):
        if getattr(current_user, "user_role", None) == "admin":
            ...
            if (
                requested_user
                and requested_user.institute_id == current_user.institute_id
            ):
        ...
        jsonify(
            {
                ...
                "user": {
                    ...
                    "role": user.user_role,
                    "institute_id": (
                        str(user.institute_id) if user.institute_id else None
                    ),
                },
            }
        ),
...
def is_origin_allowed(origin):
    ...
    if origin in ALLOWED_ORIGINS:
        return True
    if (
        origin.startswith("http://34.100.213.250")
...
def add_local_cors_headers(response):
    ...
    if is_origin_allowed(origin):
        ...
        response.headers["Access-Control-Allow-Headers"] = (
            "Content-Type, Authorization, X-Institute-Id, X-Global-Institute-Id, X-Skip-Institute-Context"
        )
