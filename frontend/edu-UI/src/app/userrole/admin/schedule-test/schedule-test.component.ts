...
export class AdminScheduleTestComponent implements OnInit, OnDestroy {
  ...
  loadUsers() {
    ...
    const targetInstitute = this.getEffectiveInstituteId();
    if (targetInstitute) {
  ...
  private applyDefaultSchedulerName(date: Date = new Date()): void {
    ...
    if (this.model.startDate) {
      const parts = String(this.model.startDate).split('-');
      if (parts.length === 3) {
        ...
        if (this.model.startTime) {
          const timeParts = String(this.model.startTime).split(':');
    ...
    const dateTime = this.formatSchedulerDateTime(targetDate);
      ...
      .subscribe({
        next: (res) => {
          try {
            ...
            this.rawInstitutesList = institutes;
            this.updateAvailableTimezones();
  ...
  get filteredUserTeamsForFilter(): Array<{ id: string; name: string; department_id?: string | null; department_name?: string | null }> {
    const depKey = `${this.userTeamSearch}|${(this.selectedUserDepartments || []).join(',')}|${(this.selectedUserTeams || []).join(',')}|${(this.userTeams || []).length}`;
    return this._memoize('filteredUserTeams', depKey, () => {
      ...
      if (this.selectedUserDepartments && this.selectedUserDepartments.length > 0) {
        const selectedDepts = this.selectedUserDepartments.map((d) => String(d));
        const selectedDeptObjs = (this.userDepartments || []).filter(
          (d) => selectedDepts.includes(String(d.id)) || selectedDepts.includes(d.name)
        );
        const deptNames = selectedDeptObjs.map((d) => (d.name || '').toLowerCase().trim());
        selectedDepts.forEach((val) => {
          if (val.trim()) deptNames.push(val.toLowerCase().trim());
        });

        list = list.filter((t: any) => {
          if (Array.isArray(this.selectedUserTeams) && this.selectedUserTeams.includes(t.id)) return true;
          const teamDeptId = t.department_id ? String(t.department_id) : '';
          const teamDeptName = t.department_name ? String(t.department_name).toLowerCase().trim() : '';
          if (teamDeptId && selectedDepts.includes(teamDeptId)) return true;
  ...
  onFilterInstituteSelected(instituteId: string): void {
    ...
    const targetInstitute = this.filterInstitute || this.model.institute;
    if (targetInstitute) {
      this.loadCampusList(targetInstitute);
    }
    this.updateAvailableTimezones();
  ...
  updateAvailableTimezones(): void {
    ...
    if (instId && this.userCampuses && this.userCampuses.length > 0) {
      this.userCampuses.forEach((uc) => {
        if (
          ...
          !this.selectedUserCampuses.includes(uc.id)
        ) {
          return;
        }
        if (this.userFilters.campus_id && this.userFilters.campus_id !== uc.id) {
          return;
        }
    ...
    // Map gathered locations to timezone options
  ...
  schedule() {
    ...
    const payload: {
      ...
      end_time: endIso,
      timezone:
        this.scheduleTimingForm?.get('timezone')?.value ||
        this.model.timezone ||
        'Asia/Kolkata',
      created_by:
  ...
  applyEditOrView() {
    try {
      ...
      if (rawEdit) {
        ...
        if (e.timezone) {
          this.model.timezone = e.timezone;
        }
        ...
        this.model.testName = this.model.testName || this.model.schedulerName;
        if (v.timezone) {
          this.model.timezone = v.timezone;
        }
        if (v.start_time) {
