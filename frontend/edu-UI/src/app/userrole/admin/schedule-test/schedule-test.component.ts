...
export class AdminScheduleTestComponent {
  ...
  private getInstituteShortName(): string {
    const user = this.getSessionUser();
    const fromSession = sessionStorage.getItem('global_institute_name') || sessionStorage.getItem('institute_short_name') || '';
    const fromUser = user?.institute_short_name || user?.short_name || '';
    const selectedInstitute = (this.institutes || []).find(i => String(i.institute_id) === String(this.model.institute || ''));
    return String(fromUser || selectedInstitute?.short_name || selectedInstitute?.name || fromSession || user?.institute_name || user?.institute || '').trim();
  }
  ...
  private applyDefaultSchedulerName(date: Date = new Date()): void {
    ...
    const instituteShortName = this.getInstituteShortName();
    const dateTime = this.formatSchedulerDateTime(date);
    this.model.schedulerName = instituteShortName ? `${dateTime} - ${instituteShortName}` : dateTime;
