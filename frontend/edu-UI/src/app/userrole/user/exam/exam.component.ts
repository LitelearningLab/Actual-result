...
@Component({
  ...
  imports: [
    ...
    ConfirmStartTestDialogComponent,
...
export class UserExamComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  constructor(
    ...
    try {
      ...
      if (raw) {
        ...
        this.isSuperAdmin = ['super_admin', 'superadmin', 'super-admin'].includes(
          String(obj?.role || '').toLowerCase()
        );
      }
    } catch (e) {}
  ...
  isPass(row: UserTestRow): boolean {
    const pct =
      row.user_percentage != null
        ? row.user_percentage
        : row.user_score != null && row.total_marks
          ? (row.user_score / row.total_marks) * 100
          : null;
  ...
  isAttemptPass(att: any, row: UserTestRow): boolean {
    const pct =
      att?.percentage != null
        ? att.percentage
        : att?.score != null && row.total_marks
          ? (att.score / row.total_marks) * 100
          : null;
    const passMark = row.pass_mark != null ? row.pass_mark : 50;
    if (pct != null) {
      return pct >= passMark;
    ...
    if (att) {
      const res = String(att.result || att.feedback || '')
        .trim()
        .toLowerCase();
      if (res === 'pass' || res === 'passed') return true;
      if (res === 'fail' || res === 'failed') return false;
  ...
  viewAttemptReview(row: UserTestRow, att: any, event?: Event): void {
    ...
    const targetItem = rows.find(
      (r) => r.attemptObj === att || (att && r.attemptNumber === att.attempt_number)
    );
    ...
    if (history && history.length > 0) {
      rows = history.map((att: any, idx: number) => {
        ...
        if (isCompletedTab) {
          ...
          const attEnd = att.submitted_date
            ? this.formatAttemptDate(att.submitted_date, true)
            : attStart || '';
      ...
      const canReviewDefault = isMultipleReview
        ? row.user_review !== false && row.review_available !== false
        : !isReviewForbidden && (!!row.user_review || !!row.review_available);
      ...
      const isRowInProgress = Boolean(row.has_in_progress);
      rows = [
        {
          attemptNumber: 1,
          isFirst: true,
  ...
  loadInstitutes(): void {
    this.http.get<any>(`${API_BASE}/get-institutes`).subscribe({
      next: (res) =>
        (this.institutes = (Array.isArray(res?.data) ? res.data : []).map((item: any) => ({
  ...
  filteredInstitutes() {
    ...
    return query
      ? this.institutes.filter((item) => item.name.toLowerCase().includes(query))
      : this.institutes;
  }
  ...
  get availableScheduleNames(): string[] {
    ...
    const afterTime = this.filterCreatedAfter
      ...
      : null;
    const beforeTime = this.filterCreatedBefore
  ...
  isMultipleReviewEnabled(row: any): boolean {
    if (!row) return false;
    const m =
      row.multiple_review !== undefined
        ? row.multiple_review
        : row.multiple_reviews !== undefined
          ? row.multiple_reviews
          : row.user_multiple_review !== undefined
            ? row.user_multiple_review
            : row.multiple;
  ...
  isAttemptReviewConsumedLocally(row: UserTestRow, attNumber?: number): boolean {
    ...
    try {
      ...
      return (
        Array.isArray(list) &&
        (list.includes(String(key)) || list.includes(String(row.schedule_id || row.test_id || '')))
      );
    } catch (e) {
      return false;
    }
      ...
      this.exams.find(
        (e) =>
          (e.schedule_id &&
            ...
            String(e.schedule_id) === String(targetRow.schedule_id)) ||
  ...
  viewReview(row: UserTestRow) {
    ...
    if (
      !this.isAdminUser &&
      (row.review_mode || '').toLowerCase() === 'instant' &&
      !row.multiple_review
    ) {
      this.dialog
        .open(ConfirmInstantReviewDialogComponent, {
          width: '32.8rem',
  ...
  private openReview(row: UserTestRow) {
    try {
      ...
      if (!userId || !schedulerId) {
        try {
          notify('Missing user or test identifiers for review', 'error');
        } catch (e) {}
        return;
      }
      ...
      this.http.get<any>(url).subscribe({
        next: (res) => {
          try {
            ...
            const attempts = Array.isArray(res?.data)
              ? res.data
              : Array.isArray(res?.attempts)
                ...
                : Array.isArray(res?.data?.attempts)
                  ? res.data.attempts
                  : [];
            this.reviewAttempts = (Array.isArray(attempts) ? attempts : []).map(
              (a: any, idx: number) => {
                const items = Array.isArray(a.review)
                  ? a.review
                ...
                const mappedItems = (items || []).map((it: any) => {
                  ...
                  const selectedArr = Array.isArray(it.selected_option)
                    ...
                    : it.selected_option
                      ? [String(it.selected_option)]
                      : it.selected
                        ? Array.isArray(it.selected)
                          ? it.selected.map((s: any) => String(s))
                          : [String(it.selected)]
                        : [];
                  // determine boolean correctness for the item
                  const itemIsCorrect =
                    typeof it.is_correct !== 'undefined'
                      ? Boolean(it.is_correct)
                      : typeof it.isCorrect !== 'undefined'
                        ? Boolean(it.isCorrect)
                        : null;
                  const inferredCorrect =
                    ...
                    (normalizedOptions && normalizedOptions.length
                      ? normalizedOptions[0].option_text
                      : '');
                  return {
                    title: row.title || '',
                    ...
                    marks_awarded:
                      typeof it.marks_awarded !== 'undefined'
                        ? it.marks_awarded
                        : typeof it.marks !== 'undefined'
                          ? it.marks
                          : 0,
                    question_marks:
                      typeof it.question_marks !== 'undefined'
                        ? it.question_marks
                        : typeof it.marks !== 'undefined'
                          ? it.marks
                          : 0,
            ...
            if (targetAttemptNo) {
              const idx = this.reviewAttempts.findIndex(
                (a) => Number(a.attempt_number) === Number(targetAttemptNo)
              );
              if (idx >= 0) this.reviewSelectedAttempt = idx;
            } else if (targetAttemptId) {
              const idx = this.reviewAttempts.findIndex(
                (a) => String((a as any).attempt_id || '') === String(targetAttemptId)
              );
              if (idx >= 0) this.reviewSelectedAttempt = idx;
            }
          } catch (e) {
        ...
        error: (err) => {
          ...
          const msg =
            err?.error?.statusMessage || err?.error?.message || 'This review is not available.';
          try {
            notify(msg, 'error');
          } catch (e) {}
      ...
      try {
        notify('Failed to request review', 'error');
      } catch (e) {}
    }
  }
  ...
  ngOnDestroy(): void {
    if (this.reviewRefreshTimer) {
      clearInterval(this.reviewRefreshTimer);
      this.reviewRefreshTimer = undefined;
    }
  }

  continueTest(row: UserTestRow, att?: any, event?: Event): void {
    if (event) {
      try {
    ...
    if (!row) return;
    const scheduleId = row.schedule_id || row.test_id || '';
    const targetEx = { ...row, schedule_id: scheduleId };
    ...
    this.http.get<any>(url).subscribe({
      next: (res) => {
        ...
        this.exams = arr.map((x: any) => {
          ...
          const rawAvailable =
            typeof x.review_available !== 'undefined' && x.review_available !== null
              ? Boolean(x.review_available)
              : typeof x.user_review !== 'undefined' && x.user_review !== null
                ? Boolean(x.user_review)
                : Boolean(x.review);
          ...
          const hasInProgress =
            Boolean(x.has_in_progress) ||
            (Array.isArray(x.attempts_history) &&
              x.attempts_history.some((a: any) => a.status === 'in_progress' || a.is_in_progress));
          ...
          return {
            ...
            answered_count: x.answered_count,
            expired,
            start_time: startVal,
            end_time: endVal,
            ...
            user_percentage:
              x.user_percentage !== undefined && x.user_percentage !== null
                ? x.user_percentage
                : x.percentage !== undefined && x.percentage !== null
                  ? x.percentage
                  : null,
            user_score:
              x.user_score !== undefined && x.user_score !== null
                ? x.user_score
            ...
            user_attempt:
              x.user_attempt !== undefined ? x.user_attempt : attempted || completedByUser ? 1 : 0,
  ...
  private updateTabDataSources() {
    ...
    const isUpcoming = (t?: string) =>
      ['upcoming', 'scheduled', 'pending', 'upcomming'].includes(lc(t));
  ...
  isUpcomingTest(type?: string): boolean {
    return ['upcoming', 'scheduled', 'pending', 'upcomming'].includes(
      ...
      const optText =
        opt && opt.option_text
          ...
          : typeof opt === 'string'
            ...
            : '';
      const optId =
        opt && opt.options_id ? String(opt.options_id) : opt && opt.id ? String(opt.id) : '';
  ...
  getRowClass(item: any): string {
    if (typeof item?.is_correct === 'boolean')
      return item.is_correct ? 'row-correct' : 'row-incorrect';
    if (typeof item?.is_correct === 'number')
      return item.is_correct ? 'row-correct' : 'row-incorrect';
    return '';
  }
  ...
  optionStatus(item: any, opt: any): string {
    try {
      ...
      const isCorrect =
        opt && (Number(opt.is_correct) === 1 || opt.is_correct === 1 || opt.is_correct === true);
  ...
  confirmStartTest(ex: UserTestRow, event?: Event): void {
    ...
    if (
      window.confirm(
        'Are you sure you want to start this test? Once started, the timer will begin and cannot be paused.'
      )
    ) {
      this.launchTest(targetEx);
    }
  ...
  launchTest(ex: UserTestRow) {
    ...
    if (!scheduleId) {
      try {
        notify('Schedule id missing', 'error');
      } catch (e) {}
      return;
    }
    const userRaw = sessionStorage.getItem('user_profile') || sessionStorage.getItem('user');
  ...
  applyFilter() {
    ...
    const predicate = (row: UserTestRow) => {
      const byText =
        ...
        (row.test_id || '').toLowerCase().includes(q);
      const byPublished =
        this.filterPublished === ''
          ? true
          : this.filterPublished === 'live'
            ? !!row.published
            : !row.published;
      const byInstitute =
        !this.isSuperAdmin ||
        !this.filterInstitute ||
        String(row.institute_id || '') === String(this.filterInstitute);
